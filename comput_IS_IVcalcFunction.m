% Returns the interdaily stability (IS) and the intradaily variability (IV) statistic for time series A.
function [IS,IV] = comput_IS_IVcalcFunction(A,dt,handles)
    ...
    for i = 1:floor(N*dthours) % 1 to the number of hours of data
        ...
        Ahourly(i) = nanmean(A(floor((i-1)/dthours+1):floor(i/dthours)));
    end

    p = 24; % period in hours
    Ap = comput_EnrightPeriodogramMean(Ahourly,p);
