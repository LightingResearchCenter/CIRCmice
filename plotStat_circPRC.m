...
function plotStat_circPRC(dataVecStructure, fieldTag, timing, periodograms, j, k, path, handles)
...
%     AdjustDataMatrix(:,i) = 24-(dataVecStructure.origData{i}-12);
%     
%     % display the data on command window
%     disp(' '); disp('Adjusted data: Columns correspond to different dates, rows to individual mice')
%         %whos
%     AdjustDataMatrix
    
        dispMatrix1 = [x' dataVecStructure.mean' dataVecStructure.SD'];
        dataVecStructure.mean = 24 - (dataVecStructure.mean - 12);

        plotStat_PRC(x,dataVecStructure.mean,dataVecStructure.SD,j,style,handles)

        % display the data on command window
        dispMatrix2 = [x' dataVecStructure.mean' dataVecStructure.SD']
    ...
    %% Annotate the Gaussian distrib. and homogeneity
        ...
        for i = 1 : length(x)
            ...
            % Check if Gaussian
            if dataVecStructure.shapWilk_H(i) == 1
              str1 = '§';
             % str1 = ' ';
            else
                str1 = '';
            ...
            if dataVecStructure.btestOut_H(i) == 1
               str2 = '£';
               % str2 = ' ';
            else
                str2 = '';
    ...
    s(j) = subplot(2,1,j);
        ...
        baseline = dataVecStructure.mean(1);
            ...
            style.yLim{2} = baseline - style.yLim{2};

        % plotStat_PRC(x,dataVecStructure.mean-baseline,dataVecStructure.SD,j,style,handles)%
        % delays are positive
        plotStat_PRC(x,baseline-dataVecStructure.mean,dataVecStructure.SD,j,style,handles) %delays are negative phase shifts using this method
